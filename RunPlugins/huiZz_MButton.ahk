...
;**************************************************************************
#HotIf !WinActive("ahk_group maxApp") ;特定最大化程序下屏蔽
	MButton::HK1_MButton() ; 如不屏蔽中键原功能在前缀加~，但这样拖拽窗口时会实时激活
	return
	V1toV2_GblCode_001:
Zz_WatchMouse:
Zz_WatchMouse()
...
HK1_MButton() { ; V1toV2: HK->Func
		...
		winstat := WinGetMinMax("ahk_id " Zz_MouseWin)
		if(winstat!=1){
...
;##############################################
Zz_WatchMouse() { ; V1toV2: Lbl->Func
		global
		Zz_MButtonState := GetKeyState("MButton", "P") ? "D" : "U"
		if (Zz_MButtonState = "U") ; 释放则完成
		{
			SetTimer(Zz_WatchMouse,0)
			if(Zz_OldMouseX=Zz_MouseStartX && Zz_OldMouseY=Zz_MouseStartY){
				SendInput("{MButton}")
			}
			return
		}
		Zz_EscapeState := GetKeyState("Escape", "P") ? "D" : "U"
		if (Zz_EscapeState = "D") ; 按Esc则还原
		{
			SetTimer(Zz_WatchMouse,0)
			WinMove(Zz_OldPosX, Zz_OldPosY, , , "ahk_id " Zz_MouseWin)
			return
		}
		; 【重新定位该窗口以匹配鼠标坐标的变化：】
		CoordMode("Mouse")
		...
		WinMove(Zz_WinX+Zz_MouseX-Zz_MouseStartX, Zz_WinY+Zz_MouseY-Zz_MouseStartY, , , "ahk_id " Zz_MouseWin)
		; 更新坐标
