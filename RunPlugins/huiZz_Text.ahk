...
class RunAnyObj {
	...
	text_var_name_zz(getZz:="",varStr:="",formatStr:="",splitStr:=" ,._-|/\"){
		...
		Loop Parse, getZz, "`"`n`"", "`"`r`""
		{
			...
			if(getZzList.Length=1){ ; V1toV2: Verify V2 Length value = V1 MaxIndex
				textResult.=getZzLoop . "`n"
				continue
			}
			lastFormat:=RegExReplace(formatLoop, ".*(\{[^{}]*\})", "$1")
			RegExReplace(formatLoop, "\{.*?\}", "", &formatCount)
			if(getZzList.Length-formatCount>0){ ; V1toV2: Verify V2 Length value = V1 MaxIndex
				Loop "getZzList.Length-formatCount" ; V1toV2: Verify V2 Length value = V1 MaxIndex
				{
	...
	text_pan_open(getZz:="",autoClear:=0){
		...
		if(IsObject(url)){
			url:=RegExReplace(url[0], "i)^(yun|pan)\.baidu\.com\/s\/")
			code:=this.text_pan_code(getZz,autoClear)
			if(code!=""){
				Run("https://pan.baidu.com/s/" url[0] "?pwd=" code "#" code)
			}else{
				Run("https://pan.baidu.com/s/" url[0])
			}
			return
		}
		;蓝奏云
	...
	text_pan_code(getZz:="",autoClear:=0){
		...
		if(IsObject(code)){
			code:=RegExReplace(code[0], codereg)
		}
		if(!IsObject(code)){
			RegExMatch(getZz, "i)(?<![0-9a-zA-Z])([0-9a-zA-Z]{4,5})(?![0-9a-zA-Z:])", &code)
		}
		if(IsObject(code) || autoClear){
			A_Clipboard:=code[0]
		}
		return code[0]
	}
	...
	text_run_url(getZz:="",browserApp:=""){
		...
		Loop Parse, getZz, "`"`n`"", "`"`r`""
		{
			...
			if(IsObject(S_LoopField)){
				Run(browserApp "" S_LoopField[0])
			}
		}
	}
	;[复制或输出文件文本的内容]
	...
	text_file_content(getZz:="",isSend:=0,encoding:=""){
		if(encoding!=""){
			try{
				...
				MsgBox("请设置正确的编码读取!`n参考：https://wyagd001.github.io/zh-cn/docs/commands/FileEncoding.htm"
				. "`n`n出错命令：" e.What "`n错误代码行：" e.Line "`n错误信息：" e.extra "`n" e.message, "文件编码出错", 16)
			}
		}
		fileVar := FileRead(getZz)
		if(fileVar!="")
	...
	text_compare_zz(getZz:="",compareApp:=""){
		if(compareApp){
			...
			FileAppend(A_Clipboard, A_Temp "\" fc ".txt")
	...
	text_remove_repeat(getZz:=""){
		...
		{
			getZzLoop:=A_LoopField
			textResultObj[getZzLoop]:=A_Index ;  V1toV2: Invalid Index errors?, try 'textResultObj.Push(<val>)' ;  V1toV2: Invalid Index errors?, try 'textResultObj.Push(<val>)'
...
n2c(n){
	...
	if StrLen(c)>(max:=2*b.Length+1) ; V1toV2: Verify V2 Length value = V1 MaxIndex
		d:=SubStr(c, 1, -max+2), c:=SubStr(c, ((-max+3)<1 ? (-max+3)-1 : (-max+3))<1 ? ((-max+3)<1 ? (-max+3)-1 : (-max+3))-1 : ((-max+3)<1 ? (-max+3)-1 : (-max+3)))
...
;中文转数字   by FeiYue
c2n(c){
	static a:=Map("零",0,"一",1,"二",2,"两",2,"三",3,"四",4,"五",5
		,"六",6,"七",7,"八",8,"九",9,"十",10,"百",100,"千",1000
		,"万",10000,"亿",100000000,"兆",1000000000000,"京",10000000000000000,"垓",100000000000000000000)
	c:=RegExReplace(c, "[[:ascii:]]")
	c:=SubStr(c, 1, 1)="十" ? "一" c:c
	r:=StrSplit(c), q:=w:=bak:=1, n:=0
	Loop r.Length
		if (v:=Round(a[r[r.Length-A_Index+1]]))>1000
...
URI_Encode(Str, All := False)
{
	Try
	{
		Static doc := ComValue("HTMLfile")
		doc.write("<body><script>document.body.innerText = encodeURI" . (All ? "Component" : "") . "(`"" . Str . "`");</script>")
		Return (AHKv1v2_Temp := (AHKv1v2_Temp := doc.body.innerText, doc.body.innerText := "", AHKv1v2_Temp), AHKv1v2_Temp) ; V1toV2: Wrapped Multi-statement return with parentheses ; V1toV2: Wrapped Multi-statement return with parentheses
	}
}
;[URI编码转换为中文]
URI_Decode(Str)
...
{	; by https://github.com/cocobelgica/AutoHotkey-JSON
	while RegExMatch(cnStr, "[^\x20-\x7e]", &ch) {
		ustr := Ord(ch[0]), esc_ch := "\u", n := 12
		while (n >= 0)
			esc_ch .= Chr((x:=(ustr>>n) & 15) + (x<10 ? 48 : 55))
			, n -= 4
		cnStr := StrReplace(cnStr, ch[0], esc_ch)
	}
	return cnStr
}
;[Unicode编码转换为中文]
...
encbin := Buffer(enclen, 0) ; V1toV2: if 'encbin' is a UTF-16 string, use 'VarSetStrCapacity(&encbin, enclen)' and replace all instances of 'encbin.Ptr' with 'StrPtr(encbin)'
strput(str,encbin.Ptr,enclen/2,"utf-16")
passbin := Buffer(passlen+=mod((4-mod(passlen,4)),4), 0) ; V1toV2: if 'passbin' is a UTF-16 string, use 'VarSetStrCapacity(&passbin, passlen+=mod((4-mod(passlen,4)),4))' and replace all instances of 'passbin.Ptr' with 'StrPtr(passbin)'
strput(pass,passbin.Ptr,strlen(pass),"utf-8")
_encryptbin(encbin.Ptr,enclen,passbin.Ptr,passlen)
return _crypttobase64(encbin.Ptr,enclen)
}
...
_decryptbin(encbin.Ptr,enclen,passbin.Ptr,passlen)
...
NumPut(bin1len-A_Index*4, "uint", a+b, bin1pointer+0)
b:=(a+b)*a
}
Loop "bin2len/4"
{
...
NumPut((A_Index-1)*4, "uint", (a+b)^c, bin1pointer+0)
b:=(a+b)*a
}
}
}
...
{
a:=NumGet(bin1pointer+0, (A_Index-1)*4, "uint")
NumPut((A_Index-1)*4, "uint", a:=(a^c)-b, bin1pointer+0)
b:=(a+b)*a
}
}
b:=0
...
{
a:=NumGet(bin1pointer+0, bin1len-A_Index*4, "uint")
NumPut(bin1len-A_Index*4, "uint", a:=a-b, bin1pointer+0)
b:=(a+b)*a
}
}
...
_crypttobase64(binpointer,binlen)
{
    ...
    out := Buffer(s*2, 0) ; V1toV2: if 'out' is a UTF-16 string, use 'VarSetStrCapacity(&out, s*2)' and replace all instances of 'out.Ptr' with 'StrPtr(out)'
    DllCall("crypt32\CryptBinaryToStringW", "ptr", binpointer, "uint", binlen, "uint", 1, "ptr", out.Ptr, "uint*", &s)
    return strget(out.Ptr,"utf-16")
}

_cryptfrombase64(string,&bin)
{
    DllCall("crypt32\CryptStringToBinaryW", "wstr", string, "uint", 0, "uint", 1, "ptr", 0, "uint*", &s, "ptr", 0, "ptr", 0)
    bin := Buffer(s, 0) ; V1toV2: if 'bin' is a UTF-16 string, use 'VarSetStrCapacity(&bin, s)' and replace all instances of 'bin.Ptr' with 'StrPtr(bin)'
    DllCall("crypt32\CryptStringToBinaryW", "wstr", string, "uint", 0, "uint", 1, "ptr", bin.Ptr, "uint*", &s, "ptr", 0, "ptr", 0)
    return s
}
;------------[文本加解密]结束------------
...
GoogleTranslate(str, from := "auto", to := "en")  {
   static JS := CreateScriptObj(), _ := JS.Call(GetJScript()), __ := JS.Call("delete ActiveXObject; delete GetObject;")
...
URIEncode(str, encoding := "UTF-8")  {
   var := Buffer(StrPut(str, encoding))
