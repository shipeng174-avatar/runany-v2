...
class RunAnyObj {
	...
	system_hidefile_zz(hide:=0,sys:=0,ext:=0,refresh:=1){
		...
		RegWrite(hide, "REG_DWORD", hideFileRegPath, "Hidden")
		RegWrite(ext, "REG_DWORD", hideFileRegPath, "HideFileExt")
		RegWrite(sys, "REG_DWORD", hideFileRegPath, "ShowSuperHidden")
		if(refresh){
	...
	system_proxy_zz(serverAddress:=""){
		...
		if(serverAddress!=""){
			RegWrite(1, "REG_DWORD", InternetSettings, "ProxyEnable")
			RegWrite(serverAddress, "REG_SZ", InternetSettings, "ProxyServer")
		}else{
			RegWrite(0, "REG_DWORD", InternetSettings, "ProxyEnable")
		}
	}
	;[清空回收站]
	...
	system_batch_ping(getZz:="",splitStr:="`n"){
		...
		Loop Parse, getZzLoop, splitStr
		{
			str:=A_LoopField
			if(str!="")
				Run(A_ComSpec " /c `"title ping " str " -t & ping " str " -t`"")
		}
	}
	;[批量执行cmd命令]
...
cmdClipReturn(command,save:=0){
	...
	try{
		...
		A_Clipboard:=""
		Run(A_ComSpec " /C " command " | CLIP", , "Hide")
