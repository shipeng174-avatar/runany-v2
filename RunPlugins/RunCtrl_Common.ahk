...
class RunAnyObj {
	...
	*/
	rule_check_ini(path, section, key, value){
		regVar := IniRead(path, section, key, A_Space)
		return regVar=value
	}
	/*
	【验证注册表的值】
	...
	*/
	rule_run_today(runName){
		...
		if(FileExist(RunCtrlLastTimeIni)){
			lastRunTime := IniRead(RunCtrlLastTimeIni, "last_run_time", runName, A_Space)
		}
		...
		if(flist && flist[1]){
			...
			lastRunTime := FileGetTime(lastRun)
		}
		if(lastRunTime){
			t1 := FormatTime(A_Now, "yyyyMMdd")
			t2 := FormatTime(lastRunTime, "yyyyMMdd")
			t1 := DateDiff((t1 != "" ? t1 : A_Now), t2, 'Days')
